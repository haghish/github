...
It is just shorter and a nicer way to install Stata packages from GitHub! The `gitget` data base is updated monthly, therefore **newly created** packages will not be available in `gitget` instantly. In Stata, type **help gitget** for more information. [Click here to see the complete list of __`gitget`__ packages](https://github.com/haghish/github/blob/master/gitget.md)
